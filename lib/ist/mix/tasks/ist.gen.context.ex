defmodule Mix.Tasks.Ist.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix ist.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      mix ist.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `ist.gen.schema` for more information on
  attributes.
  """
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix ist.gen.context can only be run inside an application directory")
    end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Phoenix.eval_from(paths, "priv/templates/ist.gen.context/context.ex", binding)
      )
    end

    paths
    |> Mix.Phoenix.eval_from(
      "priv/templates/ist.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        test_file,
        Mix.Phoenix.eval_from(paths, "priv/templates/ist.gen.context/context_test.exs", binding)
      )
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/ist.gen.context/test_cases.exs", binding)
  ...
  def raise_with_help(msg) do
    ...
    mix ist.gen.html, ist.gen.json and ist.gen.context expect a
    context module name, followed by singular and plural names of
    the generated resource, ending with any number of attributes.
    For example:

        mix ist.gen.html Accounts User users name:string
        mix ist.gen.json Accounts User users name:string
        mix ist.gen.context Accounts User users name:string
